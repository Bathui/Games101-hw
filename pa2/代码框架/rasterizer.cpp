...
{   
    ...
    float crs1 = AB.x() * AP.y() - AB.y() * AP.x();// AB x AP
    float crs2 = BC.x() * BP.y() - BC.y() * BP.x();// BC x BP
    float crs3 = CA.x() * CP.y() - CA.y() * CP.x();// CA x CP

    return ((crs1 > 0 && crs2>0 && crs3 > 0)||(crs1 < 0 &&crs2 < 0 &&crs3 < 0 )) ? true : false;
}
...
rst::rasterizer::rasterizer(int w, int h) : width(w), height(h)
{
    ...
    for (int i = 0; i < 4; i++) {
        frame_buf_msaa[i].resize(4);
        depth_buf_msaa[i].resize(4);
    }
}

int rst::rasterizer::get_index(int x, int y)
{
    return (height-1-y)*width + x;// get this
}
